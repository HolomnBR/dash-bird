# Script para testar endpoints específicos da API
...
foreach ($result in $results.GetEnumerator()) {
    $status = if ($result.Value) { "✅ OK" } else { "❌ FALHOU" }
...
if ($failedTests -eq 0) {
    ...
    Write-Host "⚠️  ALGUNS ENDPOINTS FALHARAM! Verifique a configuração do servidor." -ForegroundColor Yellow
}

Write-Host ""
Write-Host "Para testar com URL diferente, use:" -ForegroundColor Cyan
Write-Host ".\test-api-endpoints.ps1 -ServerUrl 'https://seu-servidor.com'" -ForegroundColor Gray
